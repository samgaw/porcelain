defmodule PorcelainTest.GoonTest do
  ...
  test "status" do
    assert exec("date", [], out: nil) == %Result{out: nil, err: nil, status: 0}

    assert exec("date", ["rubbish"], out: nil) == %Result{out: nil, err: nil, status: 1}
  end
  ...
  test "shell" do
    ...
    input = "Alphabetical\nlist\nof\nlines\n"
    output = "AlpHABEtICAl\nlInEs\nlIst\noF\n"
    assert Porcelain.shell(cmd, in: input) == %Result{out: output, err: nil, status: 0}

    cmd = "cat >/dev/null"
    assert Porcelain.shell(cmd, in: input) == %Result{out: "", err: nil, status: 0}
  end
  ...
  test "input stream" do
    ...
    assert exec("cat", [], in: stream) == %Result{out: "input\nfrom\nfile\n", err: nil, status: 0}
  end
  ...
  test "large input" do
    ...
    assert %Porcelain.Result{out: out, status: 0} = Porcelain.exec("wc", ["-c"], in: small_input)
    assert String.trim(out) == "#{small_size}"

    assert %Porcelain.Result{out: out, status: 0} = Porcelain.exec("wc", ["-c"], in: large_input)
